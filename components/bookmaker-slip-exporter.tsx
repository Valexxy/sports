...
import { ExternalLink, Check, Zap, Flame, Sparkles } from 'lucide-react';
...
const BOOKMAKER_FOMO_PERKS: Record<string, { fomo: string; perk: string; multiplier: string }> = {
  ...
  betking: { fomo: '👑 225% Royal Boost', perk: 'Zero-Withdrawal Fees', multiplier: '1.21x' },
  betano: { fomo: '🚀 2 Goals Ahead Early Payout', perk: '100% First Deposit Match', multiplier: '1.24x' },
  parimatch: { fomo: '🎁 ₦120,000 Welcome Pack', perk: 'Daily Free Bets', multiplier: '1.23x' },
  '22bet': { fomo: '💎 100% Bonus up to ₦105,000', perk: 'Friday Reload Boost', multiplier: '1.22x' },
  msport: { fomo: '⚡ ₦500,000 Super Welcome Voucher', perk: 'Instant Automated Settlement', multiplier: '1.25x' },
};
...
export const BookmakerSlipExporter: React.FC<BookmakerSlipExporterProps> = ({ match }) => {
  ...
  const handleExport = (bookmakerId: string) => {
    ...
    universalBookmakerBridge.loadBookmakerSlip(
      bookmakerId,
      match.homeTeam,
      match.awayTeam,
      p.topPick.selection,
    ...
    <div className="p-4 sm:p-5 rounded-3xl bg-black/80 border-2 border-stadiumGreen/40 space-y-4 font-mono text-xs shadow-2xl">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-white/10 pb-3">
        <div className="flex items-center space-x-2">
          ...
          <div>
            <span className="font-black text-white text-sm block">{t('Universal 1-Click Bookmaker Slip & VIP Multipliers')}</span>
            <span className="text-[10px] text-gray-400 font-sans">{t('Auto-Generates Booking Codes with Real-Time Odds Multipliers')}</span>
          </div>
        </div>
        <span className="px-2.5 py-1 rounded-full bg-stadiumGreen/20 text-stadiumGreen border border-stadiumGreen/40 text-[10px] font-black w-fit">
          🇳🇬 Verified Nigerian Bookmakers
        </span>
      </div>

      {/* High-FOMO Bookmaker Cards Grid */}
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className="px-1.5 py-0.2 rounded bg-gold/20 text-gold font-mono font-black text-[9px]">
                  {perkData.multiplier}
                </span>
              </div>
      ...
      <p className="text-[10px] text-gray-400 font-sans text-center">
        {t('1-Tap automatically copies formatted booking code, attaches highest payout boost, & launches bookmaker.')}
      </p>
    </div>
  );
};
